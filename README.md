...
真实踩坑样板（含岗位详情图 + 发薪截图）：[`踩坑案例库/案例/2026-07-18_运营助理岗位诈骗信号案例.md`](踩坑案例库/案例/2026-07-18_运营助理岗位诈骗信号案例.md)
...
下面是拿 [`示例批次/过程归档/原始采集/01_原始_示例.jsonl`](示例批次/过程归档/原始采集/01_原始_示例.jsonl)（3 条占位符数据，含茹美达案例）当作某平台采集结果，跑通 `打分 → 过滤 → 分平台导出` 三步之后的**真实终端输出**（非手写模拟）：
...
### 1. [运营助理](https://example.com/job/demo-rumeda)
- 公司：茹美达｜城市：远程｜行业：未标注｜学历：未标注
- 薪资：80-150元/天｜匹配分：39｜远程：明确远程
```

可以看到光靠关键词打分，「茹美达」这条并不会被自动剔除（分数够进"可尝试"），这正是为什么还需要下一步的[企业核验](#企业核验维度)——真实案例见[踩坑案例库](踩坑案例库/案例/2026-07-18_运营助理岗位诈骗信号案例.md)。
...
判断维度来自真实踩坑案例，不是凭空设计的规则；规则出处见 [`配置/过滤规则.json`](配置/过滤规则.json) 的 `risk_flags` 与 [`踩坑案例库/`](踩坑案例库/)。
